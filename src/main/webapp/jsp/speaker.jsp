...
<nav class="navbar-inverse">
	...
	<div class="container">
		<form action="SpeakerSelect">
			<input class="btn btn-info dropdown-toggle" type="text" name="speaker_name" placeholder="输入你要查询的讲师的姓名">
	...
	<div class="container" style="margin-top: 20px;">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>${list.pic_url }</td>
          <td><a href="updateSpeakerSelectByID?id=${list.id}">✎</a></td>
          <td><a onclick="delCourseById('${list.id}','${list.speaker_name }')" >X</a></td>
          </tr>
        </c:forEach>
        </tbody>
      </table>
	...
	<script type="text/javascript">
		function showAddPage(){
			location.href="jsp/addSpeaker.jsp";
		}
		function delCourseById(id,title){
			...
			Confirm.show('温馨提示：', '确定要删除'+title+'么？', {
				'Delete': {
					...
					'callback': function() {
						var param={"id":id};
						$.post("deleteSpeaker",param,function(data){
							if(data=='success'){
