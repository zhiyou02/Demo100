...
<nav class="navbar-inverse">
	...
	<div class="container" style="margin-top: 20px;">
		...
		<form id="infoForm" class="form-horizontal" action="VideoShow" method="post">
			<input name="video_id" value="${video.video_id }" type="hidden">
			
			<div class="form-group">
			    <label for="subjectId" class="col-sm-2 control-label">视频标题</label>
			    <div class="col-sm-10">
					<input class="form-control" value="${video.title}" name="title" id="subjectId" placeholder="视频名称" type="text">
			    </div>
			  </div>
			<div class="form-group">
				<label for="speaker_desc" class="col-sm-2 control-label">主讲人</label>
				<div class="col-sm-10">
					<select class="form-control" name="speaker_id" id="speaker_desc" >
						<c:forEach items="${speakers}" var="sub">
							<option <c:if test="${sub.id==id}">selected</c:if>  value="${sub.id}" >${sub.speaker_name}</option>
						</c:forEach>
					</select>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-10">
					<select class="form-control" name="subject_id" id="speaker" >
						<c:forEach items="${subjects}" var="sub">
								<option <c:if test="${sub.subject_id==course_id}">selected</c:if> value="${sub.subject_id}" >${sub.subject_name}</option>
						</c:forEach>
					</select>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-10">
					<input class="form-control" value="${video.time }" name="time" id="speaker_job"  placeholder="精确到秒(正整数)" type="text">
				</div>
			</div>
			<div class="form-group">
				<label for="picurl" class="col-sm-2 control-label">封面图片地址</label>
				<div class="col-sm-10">
					<input class="form-control" value="${video.image_url }" name="image_url" id="picurl"  placeholder="具体的url" type="text">
				</div>
			</div>
			<div class="form-group">
				<label for="Void_picurl" class="col-sm-2 control-label">视频播放地址</label>
				<div class="col-sm-10">
					<input class="form-control" value="${video.video_url }" name="video_url" id="Void_picurl"  placeholder="具体的url" type="text">
				</div>
			</div>
			<div class="form-group">
				<label for="Void_DAnce" class="col-sm-2 control-label">备注</label>
				<div class="col-sm-10">
					<textarea class="form-control" id="Void_DAnce" name="detail">${video.detail}</textarea>
				</div>
			</div>
			<div class="form-group">
